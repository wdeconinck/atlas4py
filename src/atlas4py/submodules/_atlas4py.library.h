...
void pybind_submodule_library(py::module_ &m) {
    ...
    m_library.attr("finalize_at_exit") = true;
    m_library.def("initialize", []() { pybind_library_initialize(); })
             ...
             .def("register_finalize_at_exit", [m_library]() {
                auto atexit = py::module_::import("atexit");
                atexit.attr("register")(py::cpp_function([m_library]() {
                    bool finalize_at_exit = m_library.attr("finalize_at_exit").cast<bool>();
                    if (finalize_at_exit) {
